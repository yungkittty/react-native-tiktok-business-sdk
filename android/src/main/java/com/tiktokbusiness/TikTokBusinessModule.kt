  ...
  /**
    ...
    fun identify(
        ...
        try {
            ...
            promise.resolve("Identify successful")
        } catch (e: Exception) {
            promise.reject("E_IDENTIFY", "Error in identify: ${e.message}", e)
        }
    }
    ...
    fun logout(promise: Promise) {
        try {
            ...
            promise.reject("E_LOGOUT", "Error in logout: ${e.message}", e)
        }
    }
    ...
    fun trackEvent(
        ...
        try {
            ...
            promise.resolve("Standard event reported successfully")
        } catch (e: Exception) {
            promise.reject("E_TRACK_EVENT", "Error in trackEvent: ${e.message}", e)
        }
    }
        ...
        try {
            ...
            if (properties != null && properties.keySetIterator().hasNextKey()) {
                if (properties.hasKey("DESCRIPTION") &&
                    properties.getType("DESCRIPTION") == ReadableType.String
                ) {
                    builder.setDescription(properties.getString("DESCRIPTION"))
                }
                if (properties.hasKey("CURRENCY") &&
                    properties.getType("CURRENCY") == ReadableType.String
                ) {
                ...
                if (properties.hasKey("VALUE") &&
                    properties.getType("VALUE") == ReadableType.Number
                ) {
                    builder.setValue(properties.getDouble("VALUE"))
                }
                if (properties.hasKey("CONTENT_TYPE") &&
                    properties.getType("CONTENT_TYPE") == ReadableType.String
                ) {
                ...
                if (properties.hasKey("CONTENTS") &&
                    properties.getType("CONTENTS") != ReadableType.Null
                ) {
                    ...
                    if (contentsArray != null && contentsArray.size() > 0) {
                        ...
                        for (i in 0 until contentsArray.size()) {
                            ...
                            if (contentMap != null) {
                                ...
                                if (contentMap.hasKey("CONTENT_ID") &&
                                    contentMap.getType("CONTENT_ID") == ReadableType.String
                                ) {
                                    contentBuilder.setContentId(contentMap.getString("CONTENT_ID"))
                                }
                                if (contentMap.hasKey("CONTENT_CATEGORY") &&
                                    contentMap.getType("CONTENT_CATEGORY") == ReadableType.String
                                ) {
                                    contentBuilder.setContentCategory(contentMap.getString("CONTENT_CATEGORY"))
                                }
                                if (contentMap.hasKey("BRAND") &&
                                    contentMap.getType("BRAND") == ReadableType.String
                                ) {
                                    contentBuilder.setBrand(contentMap.getString("BRAND"))
                                }
                                if (contentMap.hasKey("PRICE") &&
                                    contentMap.getType("PRICE") == ReadableType.Number
                                ) {
                                    contentBuilder.setPrice(contentMap.getDouble("PRICE").toFloat())
                                }
                                if (contentMap.hasKey("QUANTITY") &&
                                    contentMap.getType("QUANTITY") == ReadableType.Number
                                ) {
                                    contentBuilder.setQuantity(contentMap.getInt("QUANTITY"))
                                }
                                if (contentMap.hasKey("CONTENT_NAME") &&
                                    contentMap.getType("CONTENT_NAME") == ReadableType.String
                                ) {
            ...
            val event = builder.build()
            TikTokBusinessSdk.trackTTEvent(event)
            promise.resolve("Content event reported successfully")
        } catch (e: Exception) {
            promise.reject("E_TRACK_CONTENT_EVENT", "Error in trackContentEvent: ${e.message}", e)
        }
    }
    ...
    fun trackCustomEvent(eventName: String, properties: ReadableMap?, promise: Promise) {
        try {
            ...
            val event = builder.build()
            TikTokBusinessSdk.trackTTEvent(event)
            promise.resolve("Custom event reported successfully")
        } catch (e: Exception) {
            promise.reject("E_TRACK_CUSTOM_EVENT", "Error in trackCustomEvent: ${e.message}", e)
        }
    }

    @ReactMethod
    fun initializeSdk(appId: String, ttAppId: String, debug: Boolean?, promise: Promise) {
        try {
            val configBuilder = TikTokBusinessSdk.TTConfig(reactApplicationContext)
                .setAppId(appId)
                .setTTAppId(ttAppId)
            ...
            TikTokBusinessSdk.initializeSdk(configBuilder, object : TikTokBusinessSdk.TTInitCallback {
                ...
                override fun fail(code: Int, msg: String) {
                    promise.reject("E_INIT", "Initialization failed: $msg (code: $code)")
                }
            })
        } catch (e: Exception) {
            promise.reject("E_INIT", "Error initializing SDK: ${e.message}", e)
        }
    }
