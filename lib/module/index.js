...
const TikTokBusinessModule = NativeModules.TikTokBusinessModule ? NativeModules.TikTokBusinessModule : new Proxy({}, {
  get() {
 ...
 */
export let TikTokEventName = /*#__PURE__*/function (TikTokEventName) {
...
export let TikTokContentEventName = /*#__PURE__*/function (TikTokContentEventName) {
 ...
 */
export let TikTokContentEventParameter = /*#__PURE__*/function (TikTokContentEventParameter) {
 ...
 */
export let TikTokContentEventContentsParameter = /*#__PURE__*/function (TikTokContentEventContentsParameter) {
 ...
 */
export const initializeSdk = async (appId, ttAppId, accessToken, debug) => await TikTokBusinessModule.initializeSdk(appId, ttAppId, accessToken, debug || false);

/**
 * Identifies the user.
 */
export const identify = async (externalId, externalUserName, phoneNumber, email) => await TikTokBusinessModule.identify(externalId, externalUserName, phoneNumber, email);

/**
 * Logs out the user.
 */
export const logout = async () => await TikTokBusinessModule.logout();

/**
 * Reports a standard event.
 * Accepts an optional eventId and additional properties.
 */
export const trackEvent = async (eventName, eventId, properties) => await TikTokBusinessModule.trackEvent(eventName, eventId || null, properties || null);

/**
 * Reports a content event.
 * Accepts the event type (e.g., "ADD_TO_CART", "CHECK_OUT") and additional properties.
 */
export const trackContentEvent = async (eventName, properties) => await TikTokBusinessModule.trackContentEvent(eventName, properties);

/**
 * Reports a custom event.
 * Builds the event using TTBaseEvent and adds provided properties.
 */
export const trackCustomEvent = async (eventName, properties) => await TikTokBusinessModule.trackCustomEvent(eventName, properties);
 ...
 */
export const trackAdRevenueEvent = async (adRevenueData, eventId) => await TikTokBusinessModule.trackAdRevenueEvent(adRevenueData, eventId || null);
export const flush = () => TikTokBusinessModule.flush();
